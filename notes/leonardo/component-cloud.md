...
**URL Handling** (cloud -> application interactions)

- `clouduiactions.cpp/h`
- `cloudurlhandler.cpp/h`: with methods such as `tryHandleAudioLink`, this module is responsible for handling cloud-application interaction such as opening a project stored in the cloud directly into Audacity via URI such as `audacity://cloud/open-audio-file` (which basically tell the file system to possibly download, locate and open the project using audacity).

**OAuth 2.0 authorization** (manages authorization)

- `oauthhttpserverreplyhandler.cpp/h`: basically it solves the following problem: once the user logs in on audio.com, how does the website pass the authorization code back to the running desktop app?
